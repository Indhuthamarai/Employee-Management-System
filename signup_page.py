...
class SignUp:
    def __init__(self, root, open_login_window):
        self.window = root
        self.open_login_window = open_login_window  # Callback to open the login window
        self.window.title("Sign Up PySeek")
        self.window.geometry("1280x800+0+0")
        self.window.config(bg="white")

        # Load background image
        try:
            self.bg_img = ImageTk.PhotoImage(file="img1.jpeg")
        except Exception as e:
            messagebox.showerror("Error", f"Background image not found: {str(e)}")
            self.bg_img = None

        # Use the loaded image in a Label, if found
        if self.bg_img:
            background = Label(self.window, image=self.bg_img)
            background.place(x=0, y=0, relwidth=1, relheight=1)
        else:
            background = Label(self.window, bg="white")
            background.place(x=0, y=0, relwidth=1, relheight=1)
    ...
    def redirect_window(self):
        self.window.destroy()
        root = Tk()
        obj = SignUp(root)  # Correctly instantiate the SignUp class
        root.mainloop()
